...
class Selection:
    def __init__(
        ...
        self.display_summary = uf.get_complete_display_summary(
            data_directory_path,
            cdna_min_length,
            cdna_max_length,
            start_sequence,
        ...
        self.sorted_cycles = sorted(self.display_summary.keys())
        ...
        self.base_cycle_sorted_peptides = uf.get_base_cycle_sorted_peptides(
            data_directory_path,
            base_cycle,
            cdna_min_length,
            cdna_max_length,
        ...
        self.base_cycle_sorted_dna = uf.get_base_cycle_sorted_dna(
            data_directory_path,
            base_cycle,
            cdna_min_length,
            cdna_max_length,
    ...
    # Get display_summary_report, which, for the n_top_peptides,
    # returns a summary table (.txt) and provides a summary grpaph (.png).
        # A. Get summary .csv and .png path.
        today = uf.get_todays_date()
        display_summary_csv_path = f"{today}_display_summary_{self.file_name}.csv"
        display_summary_png_path = f"{today}_display_summary_{self.file_name}.png"

        # B. Populate summary .csv.
        summary_file = open(display_summary_csv_path, 'w')
        summary_file.write(
        ...
        for peptide in self.base_cycle_top_sorted_peptides:
            ...
            peptide_cdna_mutants = len(
                self.display_summary[self.base_cycle][peptide])
            summary_file.write(
...
# =============================================================================
    def get_dna_mutants_summary(self):
        ...
        today = uf.get_todays_date()
        dna_mutants_csv =  f"{today}_dna_mutants_{self.file_name}.csv"
        dna_clones_analysis_png_path = (
            f"{today}_dna_mutants_c{self.base_cycle}_{self.file_name}.png")
        
        # B. Populate mutants .csv.
        dna_mutants_file = open(dna_mutants_csv, 'w')
        dna_mutants_file.write("peptide sequence,")
        for cycle in self.sorted_cycles:
            dna_mutants_file.write(f"cycle # {cycle} dna clones (#),")
        dna_mutants_file.write("\n")
        
        for peptide in self.base_cycle_top_sorted_peptides:
            dna_mutants_file.write(f"{peptide}," )
            for cycle in self.sorted_cycles:
                dna_mutants_file.write(
                    f"{str(self.dna_clones_by_cycle_by_peptide[cycle].get(peptide, 0)),}")
            dna_mutants_file.write("\n")
        dna_mutants_file.close()
...
# =============================================================================
    def get_peptides_relatedness_summary(self):
        ...
        base_cycle_dna_forest = nx.Graph()
        # to add nodes (leaves, unique dna sequences)
        # to the base_cycle_dna_forest disconnected graph
        base_cycle_dna_forest.add_nodes_from(self.base_cycle_sorted_dna)
        # Add edges between DNA sequences
        # (if hamming distance between two DNA sequences = 1)
        # to the base_cycle_dna_forest
        # so that disconnected graphs (stand alone trees) can be identified
        used_nodes = []
        for dna1 in self.base_cycle_sorted_dna:
            used_nodes += [dna1]
            for dna2 in self.base_cycle_sorted_dna:
                if ((dna2 not in used_nodes)
                    and (uf.get_hamming_distance(dna1, dna2) == 1)):
                    base_cycle_dna_forest.add_edge(
                        dna1,
                        dna2,
                        n_mutations = 1)
        # Get connected subgraphs (stand alone trees)
        # from the disconnected graph (forest).
        base_cycle_dna_trees = list(
            # nx.connected_component_subgraphs(
            nx.connected_components(
                base_cycle_dna_forest))
        
        # to convert list of dna trees into a list of peptides trees leaves
        peptides_trees_leaves = []
        for dna_tree in base_cycle_dna_trees:
            peptide_leaves = []
            for dna in dna_tree:
        ...
        today = uf.get_todays_date()
        peptides_summary_csv = f"{today}_peptide_families_{self.file_name}.csv"
        peptides_summary_png = f"{today}_peptide_families_{self.file_name}.png"
        peptides_summary_file = open(peptides_summary_csv, 'w')

        # PLOT SETUP START ========================================================
        # Set color map.
        n_colors = len(self.sorted_cycles)
        ...
        for peptide_leaves in peptides_trees_leaves:
            ...
            tree_peptides_data = {}
            tree_peptides_data[root_peptide] = [
                0,
                '',
                0,
            ...
            tree_peptides = list(peptide_tree.nodes())
            tree_peptides.remove(root_peptide)

            for peptide in tree_peptides:
            ...
            for distance_to_the_root in peptides_by_distance_to_the_root:
                ...
                equidistant_peptides = sorted(
                    ...
                    key=lambda peptide: (tree_peptides_data[peptide][2]),
                    reverse=True)
                # Predecessor count can be used to sort the peptides.
                # However, it does not seem to be useful.
                # equidistant_peptides = sorted(
                #     equidistant_peptides,
                #     key = lambda peptide: (tree_peptides_data[peptide][1]),
                #     reverse = True)
                equidistant_peptides = sorted(
                    equidistant_peptides,
                    key=lambda peptide: (tree_peptides_data[peptide][0]),
                    reverse=False)
            ...
            nx.draw_networkx(
                ...
                with_labels = False,
                #font_size = 6,
                vmin = min(self.sorted_cycles),
                vmax = max(self.sorted_cycles))
    ...
    # =============================================================================
        ...
        legend_color_dots_y = np.linspace(
            ...
            n_colors,
            endpoint=False)
        peptide_legend_colors = self.sorted_cycles
        ...
        color_labels = self.sorted_cycles
